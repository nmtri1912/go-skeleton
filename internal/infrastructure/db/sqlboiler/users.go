...
func (o *User) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"db_local\".\"users\"", updateOnConflict, ret, update, conflict, insert)
...
var UserWhere = struct {
	...
	ID:        whereHelperint{field: "\"db_local\".\"users\".\"id\""},
	Email:     whereHelperstring{field: "\"db_local\".\"users\".\"email\""},
	Password:  whereHelperstring{field: "\"db_local\".\"users\".\"password\""},
	Name:      whereHelperstring{field: "\"db_local\".\"users\".\"name\""},
	CreatedAt: whereHelpertime_Time{field: "\"db_local\".\"users\".\"created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"db_local\".\"users\".\"updated_at\""},
	DeletedAt: whereHelpernull_Time{field: "\"db_local\".\"users\".\"deleted_at\""},
}
...
var (
	userAllColumns            = []string{"id", "email", "password", "name", "created_at", "updated_at", "deleted_at"}
...
func Users(mods ...qm.QueryMod) userQuery {
	mods = append(mods, qm.From("\"db_local\".\"users\""), qmhelper.WhereIsNull("\"db_local\".\"users\".\"deleted_at\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"db_local\".\"users\".*"})
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"db_local\".\"users\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *User) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"db_local\".\"users\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"db_local\".\"users\" %sDEFAULT VALUES%s"
		}
...
func (o *User) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"db_local\".\"users\" SET %s WHERE %s",
...
func (o UserSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"db_local\".\"users\" SET %s WHERE %s",
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"db_local\".\"users\" WHERE \"id\"=$1"
	} else {
		currTime := time.Now().In(boil.GetLocation())
		o.DeletedAt = null.TimeFrom(currTime)
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"db_local\".\"users\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o UserSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"db_local\".\"users\" WHERE " +
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"db_local\".\"users\" SET %s WHERE "+
...
func (o *UserSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"db_local\".\"users\".* FROM \"db_local\".\"users\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, userPrimaryKeyColumns, len(*o)) +
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"db_local\".\"users\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
...
// Paginate returns filtered User records from the query within limit count.
func GetAllUsers(ctx context.Context, exec boil.ContextExecutor, params FilterParam, selectCols ...string) (UserSlice, int64, error) {
	userObj := []*User{}
	...
	query := fmt.Sprintf(
		"select %s from \"db_local\".\"users\" where %s and \"deleted_at\" is null LIMIT $1 OFFSET $2", sel, where,
	)
	...
	if err != nil {
		...
		return nil, 0, errors.Wrap(err, "models: unable to select from users")
	}
	...
	countQuery := queries.Raw(
		fmt.Sprintf(
			"select COUNT(*) from \"db_local\".\"users\" where %s and \"deleted_at\" is null", where,
		),
	)
	...
	if len(userAfterSelectHooks) != 0 {
		for _, obj := range userObj {
			if err := obj.doAfterSelectHooks(ctx, exec); err != nil {
				return userObj, totalCount, err
			}
		}
	}

	return userObj, totalCount, nil
}
